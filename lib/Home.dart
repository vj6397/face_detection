...
import 'package:face_dectection/utils_scanner.dart';
...
class _HomeState extends State<Home> {
  ...
  performDetectionOnStreamFrames(CameraImage cameraImage)async{
    UtilsScanner.dectect(
        image: cameraImage,
        dectectInImage: faceDetector!.processImage,
  ...
  Widget build(BuildContext context) {
    ...
    if(cameraController!=null){
      stackWidgetChildren.add(
        Positioned(
          top: 0,left: 0,
          width: size!.width,height: size!.height-250,
          child: Container(
...
class FaceDetectorPainter extends CustomPainter{
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for(Face face in faces){
      canvas.drawRect(
        Rect.fromLTRB(cameraLensDirection==CameraLensDirection.front
